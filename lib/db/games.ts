...
export async function canUploadByIp(ipHash: string): Promise<{ ok: boolean; reason?: string }> {
...
export async function isBlocked(emailHash: string, ipHash: string): Promise<boolean> {
  const supabase = createServiceClient();
