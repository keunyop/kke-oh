...
import type { GameReaction, ReactionResult } from '@/lib/games/repository';
...
type ReactionStore = Record<string, ReactionMetrics>;

const STORE_FILE_NAME = 'reactions.json';

async function readStore(filePath: string): Promise<ReactionStore> {
  try {
...
async function writeStore(filePath: string, data: ReactionStore) {
...
export async function readReactionMetrics(gameId: string): Promise<ReactionMetrics> {
...
export async function applyReactionFallback(
  gameId: string,
  nextReaction: GameReaction,
  previousReaction?: GameReaction | null
