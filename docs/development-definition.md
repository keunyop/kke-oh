...
7. 모바일 터치 환경에서도 손가락 스와이프로 좌우 이동이 실제로 동작해야 한다.
...
10. 런타임 스모크 테스트로 실제 부팅/루프/기본 상호작용 가능 여부를 검증하고 실패 시 재시도
11. HTML에 KKE-OH 리더보드 브리지 삽입
12. 썸네일 SVG 정규화 또는 기본 썸네일 결정
13. 포인트 차감 원장 기록
14. 초안 저장
15. 리더보드 활성화
16. 진행상황 UI와 완료 상태 노출
...
11. 런타임 스모크 테스트로 실제 부팅/루프/기본 상호작용 가능 여부를 검증하고 실패 시 재시도
12. 포인트 차감 원장 기록
13. 수정 결과 저장
14. AI 생성과 동일한 수준의 진행상황 UI 표시와 완료 카드/완료 메시지 표시
...
- 생성/수정 모두 모델 응답 HTML은 저장 전에 런타임 스모크 테스트를 통과해야 한다.
...
- 홈 챔피언 스트립은 drag 이후에만 click 을 억제하고, 일반 click/tap 은 게임 상세 이동으로 연결해야 한다.

### 11.6 AI 산출물 실행 검증

- `lib/games/ai-playability.ts` 같은 공용 검증기로 AI 생성/수정 HTML을 저장 전에 검사한다.
- 검증기는 inline script 실행, DOMContentLoaded/load, 시작 버튼 클릭, timer/animation frame flush 를 포함한 최소 런타임 스모크 테스트를 수행한다.
- 외부 script 의존, 문법 오류, 즉시 런타임 예외, 게임 루프/캔버스/입력 초기화 부재는 실패로 처리한다.
- 실패 시 생성기에서 재시도 프롬프트를 추가하고, 최대 재시도 후에도 실패하면 사용자에게 오류를 반환한다.
...
- 내 게임 AI 수정 완료 후에는 결과 카드와 `/my-games?notice=updated` 상태 메시지 둘 다 같은 완료 경험을 공유해야 한다.

### 11.10 공유 메타데이터 규칙

- 루트 레이아웃 메타데이터는 `metadataBase`, canonical, Open Graph, Twitter card 를 포함해야 한다.
- 루트 URL의 기본 공유 이미지는 `/icon.svg` 로 KKE-OH 로고를 사용한다.
- 게임 상세 페이지는 `generateMetadata`로 게임명, 설명, canonical, 게임 썸네일 기반 Open Graph/Twitter 메타데이터를 생성한다.
- 게임 썸네일이 없으면 루트와 동일하게 `/icon.svg` 를 fallback 공유 이미지로 사용한다.
...
- 홈 챔피언 스트립
  ...
  - 마우스 drag
  - 터치 drag
  - drag 하지 않은 click/tap 시 게임 상세 이동
- 게임 카드 말줄임 및 tooltip
  - 제목 1줄
  - 작성자 1줄
- 로그인/회원가입 단순화 레이아웃
