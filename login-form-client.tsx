...
export default function LoginFormClient({ nextPath, locale }: { nextPath: string; locale: Locale }) {
  ...
  async function submit() {
    if (mode === 'signup' && password !== passwordConfirm) {
      setError(locale === 'ko' ? '비밀번호가 서로 같지 않아요.' : 'The passwords do not match.');
      return;
    }
  ...
  return (
    <div className="login-card panel-card auth-card-centered">
      ...
      {mode === 'signup' ? (
        <label className="field-label">
          <span>{locale === 'ko' ? '비밀번호 다시 입력' : 'Confirm password'}</span>
          <input
            value={passwordConfirm}
            onChange={(event) => setPasswordConfirm(event.target.value)}
            placeholder={locale === 'ko' ? '비밀번호를 한 번 더 적어주세요.' : 'Type your password one more time.'}
            type="password"
            maxLength={80}
