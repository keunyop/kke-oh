...
function inferCategory(game) {
    ...
    if (haystack.includes('puzzle'))
        return 'Puzzle';
    if (haystack.includes('school') || haystack.includes('student'))
        return 'School Project';
    if (haystack.includes('draw') || haystack.includes('build') || haystack.includes('paint'))
        return 'Creative';
    if (haystack.includes('mobile') || haystack.includes('touch'))
        return 'Mobile Friendly';
    if (haystack.includes('fast') || haystack.includes('quick'))
        return 'Fast Play';
    if (haystack.includes('action') || haystack.includes('jump') || haystack.includes('run'))
        return 'Action';
    if (haystack.includes('arcade') || haystack.includes('score'))
        return 'Arcade';
...
function getReactionScore(game) {
...
function getFreshnessBonus(createdAt) {
...
export function sortDiscoveryGames(games) {
...
export function filterDiscoveryGames(games, query, category) {
...
function takeGames(games, count) {
    return games.slice(0, Math.max(0, count));
}
export function buildDiscoverySections(games) {
