...
function normalizeGameRecord(id, raw) {
...
class FilesystemGameStoreBase {
    ...
    async appendFeedback(id, message) {
        const gameDir = this.resolveGameDir(id);
        if (!gameDir)
            return false;
        await fs.mkdir(gameDir, { recursive: true });
...
export class FilesystemGameRepository extends FilesystemGameStoreBase {
    async listPublic() {
        const games = await this.listForAdmin(1000);
        return games.filter((game) => game.status === 'PUBLIC' &&
            !game.is_hidden &&
            game.report_count < 2);
    }
...
export class FilesystemGameAssetStore extends FilesystemGameStoreBase {
