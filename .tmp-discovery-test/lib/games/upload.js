...
function isThumbnailExtension(extension) {
    return THUMBNAIL_EXTENSIONS.has(extension);
}
function isOptimizedThumbnailPath(assetPath) {
    return path.basename(assetPath).startsWith(OPTIMIZED_THUMBNAIL_BASENAME);
}
function getPreferredThumbnailPath(thumbnailCandidates) {
    return (thumbnailCandidates.find((candidate) => isOptimizedThumbnailPath(candidate)) ??
        thumbnailCandidates.find((candidate) => /(^|\/)thumbnail\.(png|jpg|jpeg|webp|gif|svg)$/i.test(candidate)) ??
        thumbnailCandidates[0] ??
        null);
}
function getOptimizedThumbnailPath(files) {
...
async function optimizeThumbnail(file, outputPath) {
    try {
...
export function detectContentType(assetPath) {
...
export function createSingleHtmlInspection(html, thumbnail) {
    const files = [
        {
            path: 'index.html',
...
export async function createThumbnailUpload(file) {
...
export function ensureInspectionHasThumbnail(inspection, title) {
...
export async function normalizeInspectionThumbnail(inspection) {
...
export async function prepareInspectionForPublishing(inspection, title) {
...
export async function inspectZipUpload(file) {
    if (!file.size) {
...
export async function allocateGameId(storageDir, title) {
    if (!storageDir) {
...
export async function writeUploadedGame(options) {
    ...
    await fs.writeFile(path.join(gameDir, RESERVED_META_FILE), `${JSON.stringify(gameRecord, null, 2)}\n`, 'utf8');
}
