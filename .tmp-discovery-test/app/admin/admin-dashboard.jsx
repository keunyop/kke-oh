...
function getStatusLabel(game, copy) {
    if (game.status === 'REMOVED')
        return copy.removedStatus;
    if (game.is_hidden)
        return copy.hiddenStatus;
    return copy.publicStatus;
}
export default function AdminDashboard({ initialGames, locale, adminLoginId }) {
    ...
    function patchGame(gameId, action) {
    ...
    async function runAction(game, action) {
        if (action === 'delete' && typeof window !== 'undefined' && !window.confirm(copy.confirmDelete)) {
            return;
        }
    ...
    return (<div className="admin-dashboard">
      <section className="admin-summary-grid">
        {summaryCards.map((card) => (<article key={card.label} className="panel-card admin-summary-card">
      ...
      <section className="panel-card admin-toolbar">
        ...
        <div className="admin-toolbar-controls">
          ...
          <div className="admin-filter-row">
            {filters.map((item) => (<button key={item.key} type="button" className={`button-ghost admin-filter${filter === item.key ? ' is-active' : ''}`} onClick={() => setFilter(item.key)}>
                {item.label}
              </button>))}
          </div>
      ...
      {visibleGames.length ? (<section className="admin-game-grid">
          {visibleGames.map((game) => {
                ...
                return (<article key={game.id} className="panel-card admin-game-card">
                <div className="admin-game-head">
                  ...
                  <div className="admin-card-actions">
                    {showPreview ? (<a href={getGameAssetUrl(game.id, game.entry_path)} className="button-secondary" target="_blank" rel="noreferrer">
                        {copy.preview}
                      </a>) : null}
                    {showGamePageLink ? (<a href={`/game/${game.id}`} className="button-secondary" target="_blank" rel="noreferrer">
                        {copy.page}
                      </a>) : null}
                    {game.status !== 'REMOVED' ? (<button type="button" className="button-ghost" onClick={() => void runAction(game, game.is_hidden ? 'unhide' : 'hide')} disabled={isPending}>
                        {isPending ? copy.pending : game.is_hidden ? copy.unhide : copy.hide}
                      </button>) : null}
                    {game.status !== 'REMOVED' ? (<button type="button" className="button-ghost admin-danger-button" onClick={() => void runAction(game, 'delete')} disabled={isPending}>
                        {isPending ? copy.pending : copy.remove}
                      </button>) : (<span className="admin-removed-label">{copy.removed}</span>)}
                  </div>
                </div>
                ...
                {game.hidden_reason ? (<div className="admin-hidden-reason">
                    <strong>{copy.hiddenReason}</strong>
                    <p>{game.hidden_reason}</p>
                  </div>) : null}
              </article>);
            })}
        </section>) : (<section className="empty-state-card">
          <h2>{copy.empty}</h2>
        </section>)}
    </div>);
}
